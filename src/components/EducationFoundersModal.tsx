...
interface EducationFoundersModalProps {
  isOpen: boolean
  onClose: () => void
  schoolName: string
}

export function EducationFoundersModal({ isOpen, onClose, schoolName }: EducationFoundersModalProps) {
    ...
    >
      ...
      >
        ...
        <div className="flex items-center justify-between p-6 border-b border-border">
          <div>
            <h2 className="text-section font-serif">Alumni from {schoolName}</h2>
        ...
        <div className="flex-1 overflow-y-auto p-6">
          ...
          {!loading && !error && founders.length === 0 && (
            <div className="text-center py-12">
              <p className="text-body text-muted-foreground">No founders found from this school</p>
            </div>
          )}
        </div>
      </div>
