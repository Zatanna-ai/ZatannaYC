...
interface OccupationFoundersModalProps {
  isOpen: boolean
  onClose: () => void
  occupationTitle: string
}

export function OccupationFoundersModal({ isOpen, onClose, occupationTitle }: OccupationFoundersModalProps) {
  const [founders, setFounders] = useState<Founder[]>([])
  const [loading, setLoading] = useState(false)
  const [error, setError] = useState<string | null>(null)

  useEffect(() => {
    if (isOpen && occupationTitle) {
      fetchFounders()
    }
  }, [isOpen, occupationTitle])
  ...
  async function fetchFounders() {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        console.error('[OccupationFoundersModal] API error:', response.status, errorText)
        throw new Error(`Failed to fetch founders: ${response.status} ${response.statusText}`)
      }

      const data = await response.json()
      console.log('[OccupationFoundersModal] Response:', data)
      if (data.success) {
      ...
      console.error('[OccupationFoundersModal] Error fetching founders:', err)
    } finally {
      setLoading(false)
    }
  }
    ...
    >
      ...
      >
        ...
        <div className="flex items-center justify-between p-6 border-b border-border">
          <div>
            <h2 className="text-section font-serif">Founders with role: {occupationTitle}</h2>
        ...
        <div className="flex-1 overflow-y-auto p-6">
          ...
          {!loading && !error && founders.length === 0 && (
            <div className="text-center py-12">
              <p className="text-body text-muted-foreground">No founders found with this occupation</p>
            </div>
          )}
        </div>
      </div>
