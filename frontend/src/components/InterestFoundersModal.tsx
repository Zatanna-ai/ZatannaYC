...
interface InterestFoundersModalProps {
  isOpen: boolean
  onClose: () => void
  interestName: string
}

export function InterestFoundersModal({ isOpen, onClose, interestName }: InterestFoundersModalProps) {
  ...
  async function fetchFounders() {
    ...
    setError(null)
    try {
      const encodedInterest = encodeURIComponent(interestName)
      const url = `${API_BASE_URL}/api/v1/yc-dashboard/interests/${encodedInterest}/founders?case_session_id=${YC_CASE_SESSION_ID}`

      console.log('[InterestFoundersModal] Fetching from:', url)
      const response = await fetch(url)

      if (!response.ok) {
        const errorText = await response.text()
        console.error('[InterestFoundersModal] API error:', response.status, errorText)
        throw new Error(`Failed to fetch founders: ${response.status} ${response.statusText}`)
      }

      const data = await response.json()
      console.log('[InterestFoundersModal] Response:', data)
      if (data.success) {
      ...
      console.error('[InterestFoundersModal] Error fetching founders:', err)
    } finally {
      setLoading(false)
    }
  }
    ...
    >
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {founders.map((founder) => (
                <Link
                  ...
                  className="archival-card p-4 hover:border-moss-green transition-colors flex items-center gap-4"
                >
          ...
          {!loading && !error && founders.length === 0 && (
            <div className="text-center py-12">
              <p className="text-body text-muted-foreground">No founders found with this interest</p>
            </div>
          )}
        </div>
      </div>
